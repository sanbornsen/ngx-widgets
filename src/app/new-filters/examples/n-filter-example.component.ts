...
@Component({
  ...
  selector: 'new-filter-example',
  styleUrls: ['./n-filter-example.component.scss'],
  templateUrl: './n-filter-example.component.html'
})
export class NewFilterExampleComponent implements OnInit {
  allItems: any[];
  items: any[];
  ...
  applyFilters(filters: NewFilter[]): void {
    this.items = [];
  ...
  matchesFilter(item: any, filter: NewFilter): boolean {
    let match = true;
  ...
  matchesFilters(item: any, filters: NewFilter[]): boolean {
    let matches = true;
